function [A2, A_str, real_digitsL, real_digitsR, imag_digitsL, imag_digitsR]=sd_round(A, N, flag2, mult, SI_prefixes, units, sep)
...
% % 
% % units='';  % This is a string input for the units of the data
% %            % This assumes that all of the number have the same units.  
% %            % default is empty string, '';
% % 
% % sep='';    % This is the separator string which is separates every 
% %            % three digits to the left of the decimal point.  
% %            % An empty string does not input any separators.  
% %            % default is empty string, '';
% % 
% % *********************************************************************
% %
...
% % 
% % modified 12 April     2019   Added options for units and three digit 
% %                              separator.  Updated Comments.
% %
% %
% %
% %
...
if (nargin < 6 || isempty(units)) || ~ischar(units)
    units='';
end

if (nargin < 7 || isempty(sep)) || ~ischar(sep)
    sep='';
end
    ...
    if isreal(A)
        ...
        for e1=1:m1
            for e2=1:n1
                ...
                if ~isempty(sep)
                    [aa2] = str_comma_sep(aa2, sep);
                end
                
                if isequal( SI_prefixes, 1)
                    if ~isempty(units)
                        A_str{e1, e2}=[aa2, ' ', letters{regime(e1, e2)+9}, units];
                    else
                        A_str{e1, e2}=[aa2, letters{regime(e1, e2)+9}];
                    end
                else
                    if ~isempty(units)
                        A_str{e1, e2}=[aa2, ' ', units];
                    else
                        A_str{e1, e2}=aa2;
                    end
                end
        ...
        for e1=1:m1
            for e2=1:n1
                ...
                if ~isempty(sep)
                    [aa1] = str_comma_sep(aa1, sep);
                    [aa2] = str_comma_sep(aa2, sep);
                end
                
                if isequal( SI_prefixes, 1)
                    if ~isempty(units)
                        A_str{e1, e2}= [aa1, letters{regimeR(e1, e2)+9} aa, aa2, letters{regimeI(e1, e2)+9} 'i ', units];
                    else
                        A_str{e1, e2}= [aa1, letters{regimeR(e1, e2)+9} aa, aa2, letters{regimeI(e1, e2)+9} 'i'];
                    end
                else
                    if ~isempty(units)
                            A_str{e1, e2}= [aa1, aa, aa2, 'i ' , units];
                    else
        ...
        A2=A2r.*10.^(3.*regimeR)+1i*A2i.*10.^(3.*regimeI);
